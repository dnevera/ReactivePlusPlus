...
namespace rpp::details::observers
{
    ...
    class type_erased_observer final : public observer_vtable<rpp::utils::extract_observer_type_t<TObs>>
    {
        using Type = rpp::utils::extract_observer_type_t<TObs>;
        using Base = observer_vtable<Type>;

        static constexpr const TObs& cast(const void* ptr)
        {
            return static_cast<const type_erased_observer*>(static_cast<const observer_vtable<Type>*>(ptr))->m_observer;
        }

        static constexpr TObs& cast(void* ptr)
        {
            return static_cast<type_erased_observer*>(static_cast<observer_vtable<Type>*>(ptr))->m_observer;
        }

    public:
        type_erased_observer(TObs&& observer)
            : Base{
                  .on_next_lvalue = +[](const void* b, const Type& v) { cast(b).on_next(v); },
                  .on_next_rvalue = +[](const void* b, Type&& v) { cast(b).on_next(std::move(v)); },
                  .on_error       = +[](const void* b, const std::exception_ptr& err) { cast(b).on_error(err); },
                  .on_completed   = +[](const void* b) { cast(b).on_completed(); },
                  .set_upstream   = +[](void* b, const rpp::disposable_wrapper& d) { cast(b).set_upstream(d); },
                  .is_disposed    = +[](const void* b) { return cast(b).is_disposed(); }
              }
